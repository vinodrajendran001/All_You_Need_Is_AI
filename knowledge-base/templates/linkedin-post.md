---
type: linkedin-post
created:
updated:
tags:
  - post
source_ids: []
pages_used: []
topics: []
covers_from:
covers_through:
status: draft
---

# {{post_title}}

<!-- Filename convention: `YYYY-MM-DD Post Slug.md`, dated by the day the post is drafted.
     `covers_from` / `covers_through` are the ingest window this post was selected from; the next
     run reads `covers_through` from the newest post to find its own window start.
     `pages_used` lists every wiki page the post's claims are traceable to — it is both the
     fact-check basis and the anti-repeat record. The first entry is the spine page, which enters
     a 6-week cooldown. `source_ids` lists the underlying sources so attribution stays honest.
     `topics` is 2-4 short lowercase phrases used for dedupe against the archive.
     `status`: draft -> ready -> published. -->

## Post

<!-- Paste-ready text. Nothing above or below it should need editing before it goes live.

     Anatomy:
     1. Hook          - 1-2 lines, no throat-clearing. Chosen from the variants below.
     2. The turn      - the surprising thing, stated plainly.
     3. The mechanism - why it is true, anchored to one number or one named technique.
                        This is the "reader learns something" payload; it is not optional.
     4. The analogy   - exactly one, load-bearing, not decorative. This is where the fun lives.
     5. The caveat    - what the vault records as uncertain, self-reported, or contradicted.
                        Credibility feature, not a hedge. Never skip it to make a cleaner story.
     6. Takeaway      - one line a reader could act on.
     7. Question      - one genuine question for the comments. Not "thoughts?".

     Voice: technical but accessible. Lead with the analogy or the surprise, then go deep enough
     that a practitioner learns something too. Write like a smart person explaining to a smart
     friend from another specialty.

     Length: 150-250 words. Short paragraphs, mostly 1-3 lines. Line breaks are the formatting.

     Banned: emoji spam, "game changer", "mind-blowing", "the future is here", "let that sink in",
     "I'm excited to share", rhetorical-question openers, hustle-bait, invented personal anecdotes,
     manufactured urgency, and any claim not traceable to `pages_used`.

     Allowed and encouraged: dry humour, a mild contrarian framing when the vault supports it,
     admitting what is still unknown, naming the thing that surprised you specifically. -->

## Hook variants

<!-- Three openings in different registers - e.g. the counterintuitive number, the analogy, the
     myth-correction. Mark the recommended one and say in one line why it fits this idea. -->

1.
2.
3.

**Recommended:**

## Why this topic

<!-- Why this angle beat the other candidates in the window. Record the shortlist and the scores
     on surprise / concreteness / reach / freshness, so later runs can see what was passed over
     and why. Prefer angles the vault derived across sources over facts one source stated. -->

## Fact check

<!-- Blocking. The post is not `ready` until this is complete.
     - Every factual sentence traced to a page in `pages_used`.
     - Every number matches the vault verbatim, including units and conditions.
     - Every attribution matches the source summary's `source_author`.
     - Anything the vault flags as uncertain, self-reported, unablated, or contradicted is hedged
       in the post itself, not just noted here.
     - Unverifiable claims are cut, not softened. Record what was cut and why. -->

## Attribution

<!-- Original author and link, taken from the source summary's `source_author` / `source_url`.
     Credit the source in the post body, not only here. -->

## Hashtags

<!-- 3-5, specific over generic. Prefer #ContextEngineering or #SpeculativeDecoding
     over #AI / #Innovation / #Tech. -->

## Related pages
