...
- **Normalized all 8 raw captures**: 5 renamed to `YYYY-MM-DD Author - Title.md`, `source_id` assigned to all 8, `type: raw-source` and `source/raw` added where missing, `clippings` preserved. Two captures had an empty `author:` field and were attributed to their publishing organization (Halo Research, Anthropic) with the body byline recorded on the summary instead.
- Integrated into **22 existing pages** and verified reciprocity: every page listed under a summary's `## Affected pages` cites that summary's `source_id` or links back to it. Index, log, and overview agree at 208 source IDs.
