...
<!-- One idea per file, written for two platforms. Filename convention: `YYYY-MM-DD Post Slug.md`,
     dated by the day the post is drafted.
     `covers_from` / `covers_through` are the ingest window this post was selected from; the next
...
<!-- Paste-ready text. Nothing above or below it should need editing before it goes live.
     ...
     manufactured urgency, and any claim not traceable to `pages_used`.

     Allowed and encouraged: dry humour, a mild contrarian framing when the vault supports it,
     admitting what is still unknown, naming the thing that surprised you specifically. -->
...
<!-- Three openings in different registers - e.g. the counterintuitive number, the analogy, the
     myth-correction. Mark the recommended one and say in one line why it fits this idea.
     Note if the X variant should lead with a different one; the platforms reward different hooks. -->
...
<!-- Blocking, and it covers BOTH variants. The post is not `ready` until this is complete.
     - Every factual sentence traced to a page in `pages_used`.
     - Every number matches the vault verbatim, including units and conditions.
     - Every attribution matches the source summary's `source_author`.
     - Anything the vault flags as uncertain, self-reported, unablated, or contradicted is hedged
...
<!-- Original author and link, taken from the source summary's `source_author` / `source_url`.
     Credit the source in both post bodies, not only here. -->

## Hashtags

<!-- LinkedIn: 3-5, specific over generic. Prefer #ContextEngineering or #SpeculativeDecoding
     over #AI / #Innovation / #Tech.
     X: 0-1, usually none. -->
