...
- **The tool roster is measurable in wall clock, and the mechanism is not prompt length.** On one task, median of six fresh sessions: **36.6s with five essential tools, 37.0s and 42.2s for two full-roster harnesses.** Named mechanism is constrained decoding — every schema joins the grammar the sampler must satisfy. That gives the vault's schema-versus-code debate a decision rule, **"bounded operation set: schema; open-ended operation set: code surface"**, and reframes the [[Model Context Protocol]] cost as *residency* rather than the protocol. New page [[Tool Roster Economics]].
